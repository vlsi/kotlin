...
abstract class AbstractScriptConfigurationTest : KotlinCompletionTestCase() {
    ...
    private fun createFileAndSyncDependencies(scriptFile: File) {
        ...
        val shouldBeHighlighted = KotlinHighlightingUtil.shouldHighlight(myFile) ||
                myFile.virtualFile.getUserData(IdeScriptReportSink.Reports)?.any { it.severity == ScriptReport.Severity.FATAL } == true
        assert(shouldBeHighlighted) { "Highlighting is switched off for ${myFile.virtualFile.path}" }
    }
